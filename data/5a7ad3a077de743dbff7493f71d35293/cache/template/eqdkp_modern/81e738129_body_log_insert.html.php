...
<div class="contentHeader">
	<h1 class="contentTitle">' . ((isset($this->_data['.'][0]['L_DATA_SOURCE'])) ? $this->_data['.'][0]['L_DATA_SOURCE'] : (($this->lang('DATA_SOURCE')) ? $this->lang('DATA_SOURCE') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'DATA_SOURCE'))) . '         }')) . '</h1>
</div>
...
<form method="post" action="' . ((isset($this->_data['.'][0]['ACTION'])) ? $this->_data['.'][0]['ACTION'] : '') . '" name="post">
	...
	<tr>
		<td width="50%" align="center"><button id="show_log_form" class="mainoption" type="button"><i class="fa fa-plus"></i> ' . ((isset($this->_data['.'][0]['L_INSERT'])) ? $this->_data['.'][0]['L_INSERT'] : (($this->lang('INSERT')) ? $this->lang('INSERT') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'INSERT'))) . '         }')) . '</button></td>
		<td width="50%" align="center"><button id="show_log_form" class="mainoption" name="checkraid" type="submit" value="recalc" ' . ((isset($this->_data['.'][0]['DISABLED'])) ? $this->_data['.'][0]['DISABLED'] : '') . '><i class="fa fa-refresh"></i> ' . ((isset($this->_data['.'][0]['L_CONTINUE_OLD'])) ? $this->_data['.'][0]['L_CONTINUE_OLD'] : (($this->lang('CONTINUE_OLD')) ? $this->lang('CONTINUE_OLD') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'CONTINUE_OLD'))) . '         }')) . '</button></td>
	</tr>
	<tr id="log_form" style="display:none">
		<td colspan="2" align="center">
			<fieldset class="settings">
			<dl>
				<dt><label>' . ((isset($this->_data['.'][0]['L_parser'])) ? $this->_data['.'][0]['L_parser'] : (($this->lang('parser')) ? $this->lang('parser') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'parser'))) . '         }')) . '</label></dt>
				<dd>' . ((isset($this->_data['.'][0]['PARSER_DD'])) ? $this->_data['.'][0]['PARSER_DD'] : '') . '</dd>
			</dl>
			</fieldset>
			<textarea name="log" rows="20" id="log" style="width: 99%" class="post"></textarea><br />
		<button type="submit" name="checkraid" value="submit"><i class="fa fa-check"></i> ' . ((isset($this->_data['.'][0]['L_SEND'])) ? $this->_data['.'][0]['L_SEND'] : (($this->lang('SEND')) ? $this->lang('SEND') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'SEND'))) . '         }')) . '</button>
	</tr>
	</table>
</form>
';// ELSE
