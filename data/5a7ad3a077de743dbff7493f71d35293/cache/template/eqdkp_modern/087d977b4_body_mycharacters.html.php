...
echo '
	...
	<ul class="breamcrumb">
		...
		<li class="current"><a>' . ((isset($this->_data['.'][0]['L_manage_members_titl'])) ? $this->_data['.'][0]['L_manage_members_titl'] : (($this->lang('manage_members_titl')) ? $this->lang('manage_members_titl') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'manage_members_titl'))) . '         }')) . '</a></li>
	</ul>
	<div class="clear"></div>
</div>

<form method="post" action="' . ((isset($this->_data['.'][0]['ACTION'])) ? $this->_data['.'][0]['ACTION'] : '') . '" name="data" id="charactersform">
<input type=\'hidden\' name=\'delete_id\' id=\'delete_id\' value=\'\' />
' . ((isset($this->_data['.'][0]['CSRF_TOKEN'])) ? $this->_data['.'][0]['CSRF_TOKEN'] : '') . '
</form>
<div class="contentHeader">
	<h1 class="contentTitle">' . ((isset($this->_data['.'][0]['L_manage_members_titl'])) ? $this->_data['.'][0]['L_manage_members_titl'] : (($this->lang('manage_members_titl')) ? $this->lang('manage_members_titl') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'manage_members_titl'))) . '         }')) . '</h1>
</div>
';// IF S_SHOW_NO_CONN_INFO
if ($this->_data['.'][0]['S_SHOW_NO_CONN_INFO']) { 
echo '
<div class="infobox infobox-large infobox-blue clearfix">
	<i class="fa fa-info-circle fa-4x pull-left"></i> ' . ((isset($this->_data['.'][0]['L_no_connected_char_info'])) ? $this->_data['.'][0]['L_no_connected_char_info'] : (($this->lang('no_connected_char_info')) ? $this->lang('no_connected_char_info') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'no_connected_char_info'))) . '         }')) . '
</div>
<br >
';// ENDIF
}
// IF CONNECT_CHARS
if ($this->_data['.'][0]['CONNECT_CHARS']) { 
echo '
<form method="post" action="' . ((isset($this->_data['.'][0]['ACTION'])) ? $this->_data['.'][0]['ACTION'] : '') . '">
	' . ((isset($this->_data['.'][0]['L_associated_members'])) ? $this->_data['.'][0]['L_associated_members'] : (($this->lang('associated_members')) ? $this->lang('associated_members') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'associated_members'))) . '         }')) . ': ' . ((isset($this->_data['.'][0]['JS_CONNECTIONS'])) ? $this->_data['.'][0]['JS_CONNECTIONS'] : '') . ' <input type="submit" name="connection_submit" id="connection_submit" value="' . ((isset($this->_data['.'][0]['L_uc_connectme'])) ? $this->_data['.'][0]['L_uc_connectme'] : (($this->lang('uc_connectme')) ? $this->lang('uc_connectme') : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'uc_connectme'))) . '         }')) . '" class="mainoption bi_ok" />
	' . ((isset($this->_data['.'][0]['CSRF_TOKEN'])) ? $this->_data['.'][0]['CSRF_TOKEN'] : '') . '
</form>

' . ((isset($this->_data['.'][0]['ADD_MENU'])) ? $this->_data['.'][0]['ADD_MENU'] : '') . '
';// ENDIF
}
echo ' 
<table class="table fullwidth hptt colorswitch scrollable-x">
	' . ((isset($this->_data['.'][0]['CHAR_LIST'])) ? $this->_data['.'][0]['CHAR_LIST'] : '') . '
</table>

<div class="center pagination_wrapper">
	' . ((isset($this->_data['.'][0]['CHAR_PAGINATION'])) ? $this->_data['.'][0]['CHAR_PAGINATION'] : '') . '
</div>
';// ELSE
} else {
echo '<!DOCTYPE html>
